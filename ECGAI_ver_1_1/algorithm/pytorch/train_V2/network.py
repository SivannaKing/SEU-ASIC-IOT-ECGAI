...
class Net(nn.Module):
    ...
    def forward(self, x):
        x = F.max_pool1d(torch.clamp(F.relu(self.conv1(x)), 0, 2), 8, 4)
        #x = self.bn1(x)
        x = F.max_pool1d(torch.clamp(F.relu(self.conv2(x)), 0, 2), 4, 2)
        #x = self.bn2(x)
        x = F.max_pool1d(torch.clamp(F.relu(self.conv3(x)), 0, 1), 5, 2)
        #x = self.bn3(x)
        x = F.max_pool1d(torch.clamp(F.relu(self.conv4(x)), 0, 1), 4, 2)
        #x = self.bn4(x)
        x = F.max_pool1d(torch.clamp(F.relu(self.conv5(x)), 0, 1), 2, 2)
        #x = self.bn5(x)
        x = F.max_pool1d(torch.clamp(F.relu(self.conv6(x)), 0, 1), 2, 2)
        #x = self.bn6(x)
        x = F.max_pool1d(torch.clamp(F.relu(self.conv7(x)), 0, 1), 2, 2)
        #x = self.bn7(x)
        x = self.dropout1(x)
        x = x.view(-1, 3 * 72)
        x = self.dense1(x)
