...
Bi_domain = [2^16 2^17 2^18 2^19];
Bo_domain = [2^14 2^15 2^16 2^17 2^18 2^19];
Bw_domain = [2^12 2^13 2^14 2^15 2^16];
...
%                     Bw = K * K * Mt * Nt;
                ...
                %% Phrase 2.3.1:ainIR aoutIR aweiIR binIR boutIR bweiIR numMemAccBufIR numMemAccDramIR
                    ainIR = 1 +  ceil(N / Nt);%Ä£ÐÍÐÞÕýv2.0
                    if(N * OXt * OYt <= Bo)
                        aoutIR = 2 * ceil(M / Mt);
                    else 
    ...
    %                 end
                ...
                %% Phrase 2.4 RatePowerEfficiencyIR RatePowerEfficiencyOR RatePowerEfficiencyWR
                    RatePowerEfficiencyIR = numOps / (numMemAccDramIR * 70 + numMemAccBufIR * 0.42 + numOps * 0.06) / 16 * 10^12;
                    RatePowerEfficiencyOR = numOps / (numMemAccDramOR * 70 + numMemAccBufOR * 0.42 + numOps * 0.06) / 16 * 10^12;
                    RatePowerEfficiencyWR = numOps / (numMemAccDramWR * 70 + numMemAccBufWR * 0.42 + numOps * 0.06) / 16 * 10^12;
