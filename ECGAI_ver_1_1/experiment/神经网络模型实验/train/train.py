...
def train(args, params):
    ...
    if params.get("generator", False):
        ...
        for i in range(1, params["MAX_EPOCHS"]):
        ...
        # print curve
        plt.subplot(1, 2, 1)
        plt.plot(np.arange(0, params["MAX_EPOCHS"]), record_df["loss"], 'r')
        plt.title('loss')
        plt.subplot(1, 2, 2)
        plt.plot(np.arange(0, params["MAX_EPOCHS"]), record_df["acc"], 'r')
        plt.title('acc')
        plt.show()

        model.save(params["model_save_dir"])
...
def layers_outputs(args, params):
    ...
    model = load_model(params["model_save_dir"])  # replaced by your model name

    # put into df
    record_arr = np.array(['0', 0, 0, 0, 0, 0])
    for i in range(1, len(model.layers)):
        ...
        # max & min
        layer_outputs_max = round(np.max(layer_arr), 2)
        layer_outputs_min = round(np.min(layer_arr), 2)
        # num of output
        num_output = layer_arr.size
        # percent of negative
        num_neg = np.sum(layer_arr < 0)
        percent_neg = round(num_neg/layer_arr.size, 4)
        # absmax
        absmax = max(abs(layer_outputs_max), abs(layer_outputs_min))

        new_row = np.array([layer_name, layer_outputs_max, layer_outputs_min, num_output, percent_neg, absmax])
        record_arr = np.row_stack((record_arr, new_row))

    record_df = pd.DataFrame(record_arr, columns=["name", "max", "min", "num_output", "percent_neg", "absmax"])

    # print(record_df.loc[:, ["name", "absmax"]])
    print(record_df)
