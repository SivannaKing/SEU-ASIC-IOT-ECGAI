...
module Output_regfile (
		...
		always@( posedge clk_cal or negedge rst_cal_n) begin
			if(!rst_cal_n) begin
				...
				else if(or_cs == 6'd8) begin //每一层结束之后都复位�?下waddr
					waddr <= 5'b0;
				end
				//else if(P==4'd3)
				   //regfile[cnt_sf] <= 0;
		...
		always @(posedge clk_cal or negedge rst_cal_n) begin
			if(!rst_cal_n) 
				fst_edge <= 1'b0;
			else if(or_cs == 6'd8)//每一层结束之后都复位�?下fst_edge
				fst_edge <= 1'b0;
			else begin
				if(temp)
		...
		always@(posedge clk_cal or negedge rst_cal_n) begin
			if(!rst_cal_n) begin
				...
				else if(or_cs == 6'd8)begin //每一层结束之后都复位�?下raddr
					raddr <= 0;
				end
				else begin
					ORegfile_OvldR <= 1'b0;
		...
		assign raddr_w  = (raddr < 2*R-S && or_cs == 6'd4)? raddr + S : 6'b0; //地址只在计算状�?�的时�?�才变化
