...
module Input_Regfile(
	...
	output	reg				IR_Data_O_vld	
			...
			reg		[7:0]	Regfile	[256:0];		// 160 8-bit register files
			// Signals of write
			reg		[8:0]	wr_addr;				// Write address of Input Regfile
			// Signals of read
			reg		[8:0]	rd_addr;
			reg  	[6:0]	Bm_cnt,Bm_cnt_tmp;
//----------------------------------------------------------------------------------
...
//----------------------------------------------------------------------------------
	...
	always @(posedge clk_cal or negedge rst_cal_n)
		...
		// else if(nn_layer_cnt==4'd7 && IR_Data_I_vld && Bm_cnt[0]==0)	// wr_addr of GAP
			// wr_addr <= K*Bm_cnt;
		else if((IR_Data_I_vld&&nn_layer_cnt<=4'd7)||(IR_Data_I_vld&&nn_layer_cnt==4'd9))    //||(IR_Data_I_vld&&nn_layer_cnt==4'd9)
			wr_addr <= Bm_cnt*`Bm ;              //add num_zero to padd
                else if(IR_Data_I_vld&&nn_layer_cnt==4'd8)
			wr_addr <= wr_addr+1 ;  	// sf addd	
                else
			wr_addr <= 9'b0;
	...
	always @(posedge clk_cal or negedge rst_cal_n)
		if(!rst_cal_n)
			begin
				...
				begin
				if  (nn_layer_cnt==4'd8)
					begin
					    Regfile[wr_addr] <= IR_Data_I0;
					    Regfile[wr_addr+9'd72] <= IR_Data_I1;
					    Regfile[wr_addr+9'd144] <= IR_Data_I2;
					end
				/* if  (nn_layer_cnt==4'd9)
					begin
					    Regfile[wr_addr] <= IR_Data_I0;
					    Regfile[wr_addr+9'd8] <= IR_Data_I1;
					    Regfile[wr_addr+9'd16] <= IR_Data_I2;
						Regfile[wr_addr+9'd24] <= IR_Data_I3;
						Regfile[wr_addr+9'd32] <= IR_Data_I4;
						Regfile[wr_addr+9'd40] <= IR_Data_I5;
						Regfile[wr_addr+9'd48] <= IR_Data_I6;
						Regfile[wr_addr+9'd56] <= IR_Data_I7;
...
//----------------------------------------------------------------------------------
	...
	always @(posedge clk_cal or negedge rst_cal_n)			// should be synchronic with weight_data_Ovld
		if(!rst_cal_n)begin
			IR_Data_O_vld <= 1'b0;
			Weight_Ovld_tmp <= 1'b0;
		end
		else begin
			Weight_Ovld_tmp <= Weight_Data_Ovld;
			IR_Data_O_vld <= Weight_Ovld_tmp;
		end
	...
	always @(posedge clk_cal or negedge rst_cal_n)
		begin
			if(!rst_cal_n)
				...
				rd_addr <= 8'b0;
			else if(Weight_Ovld_tmp && (rd_addr != K-1'b1))
	...
	always @(posedge clk_cal or negedge rst_cal_n) begin
		if(!rst_cal_n)			// added reset after v2.0b
			...
			if( (Weight_Ovld_tmp && (rd_addr!=K-1)) || IR_Data_O_vld)
				begin 
                                    if (nn_layer_cnt < 4'd8)
                                        begin
