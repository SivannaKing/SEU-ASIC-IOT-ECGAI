...
module PEarray16_8(     
			...
			output 					PE_OMapVld1_sf,
			output 					PE_OMapVld2_sf,
			output 					PE_OMapVld3_sf,
			output 					PE_OMapVld4_sf,
			output 					PE_OMapVld5_sf,
			output 					PE_OMapVld6_sf,
			output 					PE_OMapVld7_sf,
			output 					PE_OMapVld8_sf,			
			output  reg      			PE_end,
			//FC lyr
           ...
           wire                   PE_OMapVld1,PE_OMapVld2,PE_OMapVld3,PE_OMapVld4,PE_OMapVld5,PE_OMapVld6,PE_OMapVld7,PE_OMapVld8;
			...
			always@(posedge clk_cal or negedge rst_cal_n)begin
				if(!rst_cal_n) begin
					...
					if(or_cs != 6'd4) //每一层结束之后都复位一下waddr
						last_addr <= 4'b0;
			...
			always@(posedge clk_cal or negedge rst_cal_n)begin
				if(!rst_cal_n)
			        ...
			        //else if (nn_layer_cnt==4'd1&&or_cs==6'd2)
			        else if (wait_tran)
			             Ocnt <= 0;
				end
			end
...
//************************************************************************************
assign PE_OMapVld1_sf = (nn_layer_cnt>4'd7)?0:PE_OMapVld1;
assign PE_OMapVld2_sf =  (nn_layer_cnt>4'd7)?0:PE_OMapVld2;
assign PE_OMapVld3_sf =  (nn_layer_cnt>4'd7)?0:PE_OMapVld3;
assign PE_OMapVld4_sf =  (nn_layer_cnt>4'd7)?0:PE_OMapVld4;

assign PE_OMapVld5_sf =  (nn_layer_cnt>4'd7)?0:PE_OMapVld5;
assign PE_OMapVld6_sf =  (nn_layer_cnt>4'd7)?0:PE_OMapVld6;
assign PE_OMapVld7_sf =  (nn_layer_cnt>4'd7)?0:PE_OMapVld7;
assign PE_OMapVld8_sf =  (nn_layer_cnt>4'd7)?0:PE_OMapVld8;
