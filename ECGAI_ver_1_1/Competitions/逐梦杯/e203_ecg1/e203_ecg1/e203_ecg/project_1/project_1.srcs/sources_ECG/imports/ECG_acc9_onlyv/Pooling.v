...
module Pooling (
		input                                   clk_cal,
		input                                   rst_cal_n,     
		input       [`eightbyte_width-1:0]      Pool_IData,
		...
		reg Pool_en;
		reg Pool_Ovld_R;
		reg signed [`byte_width-1:0] Din1,Din2,Din3,Din4,Din5,Din6,Din7,Din8;
		reg signed [`byte_width-1:0] Otemp1,Otemp2,Otemp3,Otemp4;
		...
		always@( posedge clk_cal or negedge rst_cal_n) begin
			if(!rst_cal_n) begin
				...
				if(Pool_IData_vld) begin
					Pool_en <= 1'b1;
					{Din8,Din7,Din6,Din5,Din4,Din3,Din2,Din1} <=	Pool_IData;
				end
				else
					Pool_en <= 1'b0;
			end
		...
		always@(*) begin
			if(Pool_en) begin
				...
				Otemp4 = (Din7>Din8)?Din7:Din8;	
				case(P)
					4'd8:begin
						Pool_Otemp  = ( ((Otemp1 > Otemp2)? Otemp1:Otemp2) > ((Otemp3 > Otemp4)? Otemp3:Otemp4) )?
												((Otemp1 > Otemp2)? Otemp1:Otemp2):((Otemp3 > Otemp4)? Otemp3:Otemp4);
					end
					4'd5:begin
